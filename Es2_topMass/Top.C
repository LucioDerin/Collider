...
void Top::Loop(string outFileStr = "", bool isMC = true) {
    ...
    TH1D *h_nuPtReso = new TH1D("h_nuPtReso", "", 100, -1, 1);
    ...
    TH1D *h_nuPtPosFitReso = new TH1D("h_nuPtPosFitReso", "", 100, -1, 1);
    ...
    for (Long64_t jentry = 0; jentry < nentries; jentry++) {
        ...
        //Mine:
        h_bhPtReso->Fill((bjeth.Pt() - jettruthpt[bjeth_assign_i]) / jettruthpt[bjeth_assign_i]);
        h_l0PtReso->Fill((ljet0.Pt() - jettruthpt[2]) / jettruthpt[3]);
        h_l1PtReso->Fill((ljet1.Pt() - jettruthpt[3]) / jettruthpt[3]);
        h_nuPtReso->Fill((nu.Pt() - nutruthpt)/nutruthpt);
        ...
        h_l0PtPosFitReso->Fill((posFit_ljet0.Pt() - jettruthpt[2]) / jettruthpt[2]);   
        //light jet 1
        TLorentzVector posFit_ljet1(ljet1);
        posFit_ljet1.SetPtEtaPhiM(pl1, ljet1.Eta(), ljet1.Phi(), 0);
        h_l1PtPosFitReso->Fill((posFit_ljet1.Pt() - jettruthpt[3]) / jettruthpt[3]);   

        TLorentzVector posFit_nu(nu); // NB THIS IS STILL THE PREFIT ONE!!
        // Mine:
        double nuPx = -(posFit_bjetl.Px() + posFit_bjeth.Px() + posFit_ljet0.Px() + posFit_ljet1.Px() + mu.Px());
        double nuPy = -(posFit_bjetl.Py() + posFit_bjeth.Py() + posFit_ljet0.Py() + posFit_ljet1.Py() + mu.Py());
        posFit_nu.SetPxPyPzE(nuPx, nuPy, pz, sqrt(nuPx * nuPx + nuPy * nuPy + pz*pz));
        h_nuPtPosFitReso->Fill((posFit_nu.Pt() - nutruthpt) / nutruthpt);
    ...
    c_reso->Divide(5, 1);
    //Mine:
    h_blPtPosFitReso->SetLineColor(kRed);
    h_bhPtPosFitReso->SetLineColor(kRed);
    h_l0PtPosFitReso->SetLineColor(kRed);
    h_l1PtPosFitReso->SetLineColor(kRed);
    h_nuPtPosFitReso->SetLineColor(kRed);
    ...
    c_reso->cd(3);
    h_l0PtPosFitReso->Draw("SAME");
    h_l0PtReso->Draw("SAME");
    c_reso->cd(4);
    h_l1PtPosFitReso->Draw("SAME");
    h_l1PtReso->Draw("SAME");
    c_reso->cd(5);
    h_nuPtPosFitReso->Draw("SAME");
    h_nuPtReso->Draw("SAME");
    ...
    //Mine:
    TCanvas *th_tl = new TCanvas("th_tl","th_tl",600,600);
    th_tl->Divide(2,1);
    th_tl->cd(1);
    h_tl_posFit->SetLineColor(kRed);
    h_tl_posFit->Draw("SAME");
    h_tl_preFit->Draw("SAME");
    th_tl->cd(2);
    h_th_posFit->SetLineColor(kRed);
    h_th_posFit->Draw("SAME");
    h_th_preFit->Draw("SAME");

    TCanvas *chi2_canvas = new TCanvas("chi2_canvas","chi2_canvas",600,600);
    h_chi2->Draw();
